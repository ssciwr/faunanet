...
RUN apt-get update && apt-get install --no-install-recommends -y ffmpeg git pkg-config cmake libhdf5-dev build-essential && apt-get clean && rm -rf /var/lib/apt/lists/*
